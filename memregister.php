...
if (isset($_POST['clickin'])){
   $titu = trim(strip_tags($_POST['titu']));
   $sname = trim(strip_tags($_POST['sname']));
   $onames = trim(strip_tags($_POST['onames']));
   $fon = trim(strip_tags($_POST['fon']));
   $email = trim(strip_tags($_POST['emal']));
   $dept = trim(strip_tags($_POST['dept']));
   $rank = trim(strip_tags($_POST['rank']));
   $level = trim(strip_tags($_POST['level']));
   $balance = trim(strip_tags($_POST['balance']));
   $msave = trim(strip_tags($_POST['msave']));
   $dob = trim(strip_tags($_POST['dob']));
   $datjoin = trim(strip_tags($_POST['datjoin']));

  // echo $sname.' '.$onames.' '.$fon.' '.$email.' '.$dept.' '.$rank.' '.$level.' '.$balance.' '.$msave.' '.$dob;   
  $queryy = mysqli_query($conn, "SELECT * FROM memberz where emal='$email'");
 $komo = mysqli_num_rows($queryy);

  if($titu == "" ||$sname == "" ||$onames == "" ||$fon == "" ||$email == "" ||$dept == "" ||$rank== "" ||$level == "" ||$balance == "" ||$msave == "" ||$dob == "" ||$datjoin == "") {
 ...
 $mysqlu = mysqli_query($conn, "INSERT INTO memberz (titl, sname, onames, dob, fon, email, hpazz, dept, rank, datereg, level, balance, msave, djoin) VALUES ('$titu', '$sname','$onames','$dob','$fon','$email','86ea2d4b2efc06df359c4ba2e951d0fd','$dept','$rank', '$today', $level, $balance, $msave, '$datjoin')"); 
            
  echo'<script type="text/javascript"> 
    alert("Congratulations, you have successfully registered as a member.");
     </script>';

  $uessmail = $_SESSION['email'] = $email;
  $fulln =$sname.''.$onames;
  $uefulln = $_SESSION['fulln']=$fulln;                      
      }
   }
                        ...
                        <div class="panel-body">
                           <form class="col-sm-6" method="POST" action="" >
                              <div class="form-group">
                                 <label>Title</label>
                                 <select class="form-control" name="titu">
                              ...
                              <div class="form-group">
                                 ...
                                 <input type="text" class="form-control" placeholder="Enter Other Names" name="onames" required>
                              </div>
                              <div class="form-group">
                                 <label>Email</label>
                                 <input type="email" class="form-control" placeholder="Enter Email"  name="emal" required>
                              </div>
                              <div class="form-group">
                                 <label>Phone number</label>
                                 <input type="text" name="fon" class="form-control" placeholder="Enter Mobile" required>
                              </div>
                              <div class="form-group">
                                 <label>Date of Birth</label>
                                 <input id='minMaxExample' type="date" class="form-control" name="dob" placeholder="Enter Date...">
                              </div>
                              <div class="form-group">
                              ...
                              <div class="form-group">
                                 <label>Rank</label>
                                 <input type="text" class="form-control" placeholder="Enter your designation" name="rank" required>
                              </div>
                              <div class="form-group">
                                 <label>Level</label>
                                 <input type="number" class="form-control" placeholder="Enter level" name="level" required>
                              </div>
                              <div class="form-group"> 
                                 <label>Saving Balance</label>
                                 <input type="number" class="form-control" name="balance" placeholder="Enter your saving balance" required>
                              </div>
                              <div class="form-group">
                                 <label>Monthly saving</label>
                                 <input type="number" class="form-control" placeholder="Enter monthly" name="msave" required>
                              </div>
                              <div class="form-group">
                                 <label>Date joined</label>
                                 <input type="date" class="form-control" placeholder="Enter date joined the cooperative" name="datjoin" required>
                              </div> 
                              <div class="form-group">
                                 <button type="submit" name="clickin" class="btn btn-add"><i class="fa fa-check"></i> Submit
                                 </button>
                              </div>
                           </form>
                        </div>
