<?php require_once("cann.php");
 ...
 $new = ($_GET['mont']);
  ...
  if (isset($_POST["daddy"])){
      ...
      $memmen = mysqli_query($conn, "SELECT * FROM memberz WHERE dele = 0 AND coopid= $coopidd");
  ...
  $mysqee = mysqli_query($conn, "INSERT INTO $new2 (fulln, email, msave) VALUES ('$fully','$email1', $msave1)"); 
...
<head>
    ...
    <body>
	  ...
	  <h3 class="page-title">Preview to generate bulk savings for <?php echo $mm2.' '.$yy;?></h3>
		  ...
		  <?php 
		  $kan = 1;
               $memmem = mysqli_query($conn, "SELECT * FROM memberz WHERE dele = 0  AND coopid= $coopidd ORDER BY sname ASC");
