...
//echo md5($a);
    ...
    if (isset($_POST['savemin']) || !empty($_POST['emal']) || !empty($_POST['pwd'])) {
       ...
       $bb = 'CODED'.$pazz;
       $hpazz2 = md5($bb); 
//echo $uzer.' '.$pazz.' '.$hlame2.' '.$hpazz2;
     
    $acct = mysqli_query($conn, "SELECT * FROM adminlogin WHERE hlame = '{$hlame2}' AND hpazz = '{$hpazz2}' AND aktivate= 1");
    while ($row=mysqli_fetch_assoc($acct)) {
     		$cooid = $row['id'];
     		$adlame=$row['fulln'];
     	     $uzer=$row['emal'];
     	     $hlame2=$row['hlame'];
            $cooname=$row['cooplame'];
            $activ1=$row['aktivate'];
        }
         ...
         if ($activ1== 0) {
         	header("Location:coopreg.php"); 
                              }

       else if($uzer == "" || $pazz == "") //Be sure that all the fields are filled then proceed
    {
      echo '<script type="text/javascript">
        ...
        alert("The login details provided are not correct. Please retry.");
            ...
            $result2=mysqli_query($conn, "INSERT INTO logz (mymal, typee, dreg) VALUES (
              '$uzer', 'logged_in', '{$date_reg}')") or die("Insert Error: ".mysqli_error());
             
  //    echo 'DEBE';              
            //session variables
            	$coopidd =$_SESSION['id'] = $cooid;
            	$myname= $_SESSION['namy'] = $adlame;
                $hmail =$_SESSION['hmal'] = $hlame2;
                $mymal =$_SESSION['mymal'] = $uzer;
                $cnamey =$_SESSION['coop'] = $cooname;
				...
				<div class="container">
					...
					<div class="account-box">
						<div class="account-wrapper">
							<h3 class="account-title">Coop Admin Login</h3>
