...
// 87e25944099a7715a8350f1bf1851107
   ...
   $emal1 = mysqli_real_escape_string($conn, strip_tags($_POST['emal']));
   ...
   $dobday = substr("$dob",5);
   //$dobmth = substr("$dob",5,2);


 $amm = 'Coop'.$emal1;
 $hemail = md5($amm);

 $queryy = mysqli_query($conn, "SELECT * FROM memberz where email='$emal1'");
 $komo = mysqli_num_rows($queryy);
 
  if($sname == "" || $onames == "" || $fon == "" || $emal1 == "" || $dept == "" ||$rank== "" ||$level == "" || $balance == "" || $msave == "" ||$dob == ""  || $djoin == "") {
 echo '<script type ="text/javascript">alert("please do not submit empty form."")</script>';}
 

 else if(!filter_var($emal1, FILTER_VALIDATE_EMAIL)){
     ...
     $insert1 = mysqli_query($conn, "INSERT INTO memberz (coopid, staffid, sname, onames, dob, bday, fon, email, hpazz, dept, rank, datereg, level, balance, msave, titl, djoin, hemail) VALUES ($coopidd, '$staffid', '$sname', '$onames', '$dob', '$dobday', '$fon', '$emal1', '87e25944099a7715a8350f1bf1851107', '$dept', '$rank', '$date_reg', '$level', '$balance', '$msave', '$titl', '$djoin', '$hemail')");
            
  		echo'<script type="text/javascript"> 
   		 alert("Member successfully registered. Please inform the member to login on the app 
   		 with his or her email. The default password is PASSWORD.");
      </script>';
   }
}
									 ...
									 <div class="col-sm-6 col-md-3">
										<div class="form-group">
										 <label>Email address <span class="text-danger">*</span></label>
												<input type="email" name="emal" class="form-control" required>
										</div>
									</div>
