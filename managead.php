<?php require_once("cann.php");
    ...
    $acct = mysqli_query($conn, "SELECT * FROM stafflogin WHERE emally = '$mymal'");
 ...
 $hemail = md5($amm);
    ...
    if($fulln7 == "" || $emal7 == ""|| $fon7 == "" || $rolez7 == ""){
      ...
      echo "<script type ='text/javascript'>
        ...
        $insert1 = mysqli_query($conn, "INSERT INTO stafflogin (coopid, fulname, emally, foney, hashpwd, hashmally, rolez, regby, datereg) 
        VALUES ($coopidd, '$fulln7', '$emal7', '$fon7', '87e25944099a7715a8350f1bf1851107', '$hemail', '$rolez7', '$mymal',  '$date_reg')");
...
if (isset($_POST['delet'])){
  ...
  $mysqlu2 = mysqli_query($conn, "UPDATE stafflogin SET status = 0 WHERE id = $ide1"); 
...
<head>
    ...
    <body>
        ...
        <div class="main-wrapper">
								...
								<form method="post" action="">
								  ...
								  <div class="form-group">
									...
									<div class="submit-section">
										<button  type="submit" name="clikky" class="btn btn-info submit-btn">Submit now</button>
									</div>
								</form>
							</div>
						</div>
		  ...
		  <?php 
               $memmem = mysqli_query($conn, "SELECT * FROM stafflogin WHERE status = 1 AND coopid ='$coopidd' ORDER BY fulname ASC");
                     $kan = 1;
          while ($row=mysqli_fetch_assoc($memmem)) {
            ...
            ?>  
										<tr>
											<td><?php echo $kan; ?></td>
											<td><?php echo $fulln9; ?></td>
											<td><?php echo $emal9; ?></td>
											<td><?php echo $fon9; ?></td>
											<td><?php echo $rolez9; ?></td>
